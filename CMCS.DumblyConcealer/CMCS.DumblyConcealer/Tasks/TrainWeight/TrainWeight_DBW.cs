...
{
    public class TrainWeight_DBW
    {
        private static TrainWeight_DBW instance;

        private static String MachineCode_CHSB = GlobalVars.MachineCode_CHSB;

        public static TrainWeight_DBW GetInstance()
        {
            if (instance == null)
            {
                instance = new TrainWeight_DBW();
            }
            return instance;
        }

        private TrainWeight_DBW()
        {
        }
        /// <summary>
        /// 同步报文
        ...
        public List<CmcsTrainCarriagePass> GetDBWInfo(Action<string, eOutputType> output)
        {
            ...
            String interface_dbwdest = DcDbers.GetInstance().Interface_DBWDest;
            int interface_dbwtime = DcDbers.GetInstance().Interface_DBWTime;
            int res = 0;
